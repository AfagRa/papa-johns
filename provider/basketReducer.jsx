function basketReducer(basket, action) {
    ...
    return type == 'add' ? addToBasket(basket, payload.id, payload.size, payload.quant, payload.category) :
           type == 'del' ? deleteFromBasket(basket, payload.ind) :
                           updateBasket(basket, payload.ind, payload.quant) 
}

function addToBasket(basket, id, size, quant, category) {
    let ind = basket.findIndex(item => item.id == id && item.size == size && item.category == category)
